...
response = requests.get(url="https://opentdb.com/api.php?amount=50&category=11&difficulty=easy&type=multiple")
...
def is_correct(btn):
    if btn.text() == parameters['correct'][-1]:
        ...
        parameters['attempt'][-1] -= 1
        widgets['attempt'][-1].setText(str(parameters['attempt'][-1]))
        if parameters['attempt'][-1] == 0:
            clear_widgets()
            frame4()
...
def frame1():
    ...
    clear_widgets()
    #logo widget
    image = QPixmap("static/quiz logo.png")
    logo = QLabel()
    ...
    #creator widget
    image2 = QPixmap('static/logo_gawronski.png')
    creator = QLabel()
    creator.setPixmap(image2)
    creator.setAlignment(QtCore.Qt.AlignCenter)
    creator.setStyleSheet('height: 80px; width: 80px')
    widgets['creator'].append(creator)



    grid.addWidget(widgets["logo"][-1], 0, 0, 1, 2)
    grid.addWidget(widgets["button"][-1], 1, 0, 1, 2)
    grid.addWidget(widgets['creator'][-1], 2, 0, 2, 2 )


# FRAME 2
def frame2():
    text = QLabel("Attempts:")
    text.setStyleSheet(
        '''
        font-family: 'shanti';
        font-size: 25px;
    ...
    """ game """
    #attempts widget
    attempt = QLabel(str(parameters['attempt'][-1]))
    attempt.setStyleSheet(
        """
        font-size: 35px;
        color: 'white';
        padding: 10px 20px;
    ...
    widgets['attempt'].append(attempt)


    #score widget
    score = QLabel(str(parameters['score'][-1]))  #QLabel only accept string!
    ...
    grid.addWidget(widgets['attempt'][-1], 1, 0)
    grid.addWidget(widgets["question"][-1], 2, 0, 1, 2)
...
def frame3():
    ...
    message = QLabel("Congratulations! You win!\n your score is:")
